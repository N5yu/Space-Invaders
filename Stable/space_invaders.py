...
pygame.mixer.music.load('assets/sound effects/Remi_Gallego_-_Malware_Injection_67119636.mp3')# Загрузка файлов с музыкой
...
# Уровни игры
levels = [
    [5, 2, 0],  # Уровень 1: 5 врагов, 2 пули
    [7, 2, 1],  # Уровень 2: 7 врагов, 3 пули
    [10, 3, 3]  # Уровень 3: 10 врагов, 4 пули
]
            ...
            for enemy in enemy_hits:
                if isinstance(enemy, Boss):#Проверяется, является ли враг объектом класса Boss. Если да, то выполняется следующее:
                    if not enemy.invulnerable:#Проверяется, что босс не находится в режиме "неуязвимости".
                        ...
                        if enemy.hits_taken >= 10:#Проверяется, достиг ли счетчик попаданий в босса значения 1
